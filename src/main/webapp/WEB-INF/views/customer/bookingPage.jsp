...
<body>
            ...
            <div class="bookings-container">
                ...
                <div class="mb-6">
                    ...
                    <p class="text-muted text-sm">
                        Track active reservations, check payment status, and manage automated VNPAY cancellations ().
                    </p>
                </div>
                ...
                <div class="filter-panel">
                    <form action="${pageContext.request.contextPath}/customer/bookings" method="get" class="filter-form">
                        <input type="hidden" name="action" value="search" />
                        <div class="form-group mb-0" style="flex-grow: 1;">
                            <input type="text" name="keyword" class="form-control"
                                   placeholder="Search by vehicle name, brand, license plate, booking ID or status..."
                                   value="<c:out value='${keywordVal}'/>" style="height: 42px;" />
                        </div>
                        <button type="submit" class="btn btn-blue btn-sm" style="height: 42px; padding: 0 var(--space-6);">
                            Search
                        </button>
                        <c:if test="${not empty keywordVal}">
                            <a href="${pageContext.request.contextPath}/customer/bookings?action=list"
                               class="btn btn-ghost btn-sm" style="height: 42px; display: flex; align-items: center;">
                                Reset
                            </a>
                        </c:if>
                    </form>
                ...
                <c:if test="${not empty requestScope.message}">
                    <div style="background: var(--color-dark-card); border: 1px solid var(--color-dark-border); border-radius: var(--radius-xl); padding: 3rem; text-align: center; margin: 2rem 0;">
                        <div style="margin-bottom: 1rem;"><i class="bi bi-car-front-fill" style="font-size: 3rem; color: var(--orange);"></i></div>
                ...
                <c:if test="${not empty requestScope.carBookingList}">
                    <c:forEach var="booking" items="${requestScope.carBookingList}">
                        <div class="booking-card">
                            <div class="booking-header">
                                <div>
                                    <span class="booking-ref">#BOOKING-<c:out value="${booking.bookingId}" /></span>
                                ...
                                <div>
                                    <%-- BR16: Dynamic Workflow State Badges --%>
                                    <c:choose>
                            ...
                            <div class="booking-grid">
                                <div>
                                    ...
                                    <span class="car-plate-badge"><c:out value="${booking.licensePlate}" /></span>
                                ...
                                <div>
                                    <span style="font-size: 0.75rem; text-transform: uppercase; color: var(--color-gray-light); display: block;">Duration</span>
                                    <strong style="font-size: 1.1rem; color: var(--color-white);"><c:out value="${booking.totalDays}" /> Days</strong>
                                    <span style="font-size: 0.8rem; color: var(--color-gray-mid); display: block; margin-top: 0.25rem;">
                                ...
                                <div style="text-align: right; display: flex; flex-direction: column; align-items: flex-end; justify-content: center;">
                                    <span style="font-size: 0.75rem; text-transform: uppercase; color: var(--color-gray-light); display: block;">Total Advance Value</span>
                                    ...
                                    <c:if test="${booking.status == 'Paid' || booking.status == 'Pending Payment'}">
                                        <button type="button" class="btn btn-ghost btn-sm"
                                                style="color: var(--color-red); margin-top: 0.75rem;"
    ...
    <div id="cancelModal" class="modal-overlay">
        <div class="modal-content">
            ...
            <p style="color: var(--color-gray-light); font-size: 0.9rem; margin-bottom: 1rem; line-height: 1.5;">
                Are you sure you want to cancel reservation for <strong id="cancelTargetCar" style="color: var(--color-white);"></strong>?
            </p>

            <div style="background: var(--color-dark-surface); border: 1px solid var(--color-dark-border); border-radius: var(--radius-lg); padding: var(--space-4); margin-bottom: 1.5rem; font-size: 0.85rem; color: var(--color-gold);">
                ℹ️ <strong>VNPAY Automated Refund Policy:</strong>
                <br/>
                • Cancellations requested <strong>at least 1 hour prior to pickup</strong> are eligible for a 100% automated VNPAY refund.
                <br/>
            ...
            <form action="${pageContext.request.contextPath}/customer/bookings" method="post">
                ...
                <div style="display: flex; justify-content: flex-end; gap: var(--space-3);">
                    ...
                    <button type="submit" class="btn btn-blue btn-sm" style="background: var(--color-red); border-color: var(--color-red);">
