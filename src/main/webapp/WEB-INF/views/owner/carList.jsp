                ...
                <body>
                    ...
                    <div class="mgmt-wrapper">
                        ...
                        <main class="mgmt-content">
                            <div class="admin-container">
                                ...
                                <div
                                    ...
                                    <div>
                                        ...
                                        <p class="text-muted text-sm">Manage, register and monitor active cars in your
                                            rental fleet.</p>
                                    </div>
                                    <a href="${pageContext.request.contextPath}/owner/cars?action=register"
                                        class="btn btn-primary"
                                        style="height:42px; display:flex; align-items:center; gap:0.5rem;">
                                ...
                                <div class="filter-bar">
                                    <form action="${pageContext.request.contextPath}/owner/cars" method="get"
                                        style="display:flex; flex-grow:1; margin:0; align-items:center;">
                                        <input type="hidden" name="action" value="search" />
                                        <input type="hidden" name="size" value="${pageSize != null ? pageSize : 10}" />
                                        <div class="search-wrapper" style="flex-grow:1;">
                                            <input type="text" name="keyword" value="${keywordVal}"
                                                placeholder="Search by car name or plate and press Enter..." />
                                        ...
                                        <div class="car-list-container">
                                            ...
                                            <c:forEach var="car" items="${carList}" varStatus="loop">
                                                <div class="car-list-row"
                                                    data-name="<c:out value='${car.brand} ${car.carName} ${car.model}'/>"
                                                    data-plate="<c:out value='${car.licensePlate}'/>"
                                                    data-status="<c:out value='${fn:toLowerCase(car.status == "
                                                    Pending_Approval" ? "pending" : car.status)}' />">

                                                <!-- Column 0: STT -->
                                                <div
                                                    style="font-weight: bold; font-family: monospace; color: var(--text-muted);">
                                                    ...
                                                    <c:choose>
                                                        <c:when test="${not empty car.primaryImageUrl}">
                                                            <img src="${pageContext.request.contextPath}${car.primaryImageUrl}"
                                                                class="car-thumb" alt="${car.carName}" />
                                                        </c:when>
                                                        <c:otherwise>
                                                ...
                                                <div class="col-rate">
                                                    <fmt:formatNumber value="${car.pricePerDay}" type="number"
                                                        groupingUsed="true" /> ₫ <span>/ day</span>
                                                </div>
                                                ...
                                                <div>
                                                    <span
                                                        ...
                                                        <c:out
                                                            value="${car.status == 'Pending_Approval' ? 'Pending' : car.status}" />
                                                    </span>
                                                </div>
                                                ...
                                                <div class="col-actions">
                                                    <a href="${pageContext.request.contextPath}/owner/cars?action=detail&id=${car.carId}"
                                                        class="btn-action edit" title="Edit Car">
                                                        <i class="bi bi-pencil"></i>
                                                    </a>
                                                    <button class="btn-action delete" title="Delete Car"
                                                        onclick="deleteCar(event, '${car.carId}', '<c:out value="
                            ...
                            <c:if test="${totalPages > 0}">
                                <div class="pagination-wrapper"
                                    ...
                                    <div class="pagination-links" style="display:flex; gap:0.25rem;">
                                        ...
                                        <a href="?action=${not empty keywordVal ? 'search' : 'list'}&keyword=${keywordVal}&size=${pageSize}&page=${prevPage}"
                                            class="btn btn-sm ${currentPage == 1 ? 'disabled' : ''}"
                                            style="border:1px solid var(--border); background:var(--white);">&laquo;
                                            Prev</a>

                                        <c:forEach begin="1" end="${totalPages}" var="p">
                                            <a href="?action=${not empty keywordVal ? 'search' : 'list'}&keyword=${keywordVal}&size=${pageSize}&page=${p}"
                                                class="btn btn-sm"
                                                style="${p == currentPage ? 'background:var(--primary); color:white; border-color:var(--primary);' : 'border:1px solid var(--border); background:var(--white);'}">${p}</a>
                                        </c:forEach>

                                        <c:set var="nextPage"
                                            value="${currentPage + 1 <= totalPages ? currentPage + 1 : totalPages}" />
                                        <a href="?action=${not empty keywordVal ? 'search' : 'list'}&keyword=${keywordVal}&size=${pageSize}&page=${nextPage}"
                                            class="btn btn-sm ${currentPage == totalPages ? 'disabled' : ''}"
                                            style="border:1px solid var(--border); background:var(--white);">Next
                                            &raquo;</a>
                                    </div>
                                </div>
                            </c:if>
                            ...
                            <c:otherwise>
                                <div class="db-empty"
                                    style="background:var(--white); border:1px solid var(--border); border-radius:var(--r-lg); padding:4rem 2rem; text-align:center;">
                                    <i class="bi bi-car-front-fill"
                                        style="font-size:2.5rem; color:var(--text-faint); margin-bottom:1rem; display:block;"></i>
                                    <div style="font-weight:700; color:var(--text-primary); font-size:1.1rem;">No
                                        registered cars found</div>
                                    <p style="color:var(--text-muted); font-size:var(--text-sm); margin-top:0.25rem;">
                                        Start by registering your first car on the platform.</p>
                                    <a href="${pageContext.request.contextPath}/owner/cars?action=register"
                                        class="btn btn-primary" style="margin-top: 1.5rem;">Register Now</a>
                                </div>
                            </c:otherwise>
                            </c:choose>
                    ...
                    <!-- Delete hidden form -->
                    <form id="deleteForm" action="${pageContext.request.contextPath}/owner/cars?action=delete"
                    ...
                    <script>
                        function deleteCar(event, carId, carName) {
                            ...
                            if (confirm("Are you sure you want to remove the car '" + carName + "'? This action will mark it as deleted and cancel future operations.")) {
                                ...
                                if (visibleCount === 0) {
                                    ...
                                    if (!emptyState) {
                                        ...
                                        emptyState.innerHTML = '<i class="bi bi-funnel-fill" style="font-size:2rem; color:var(--text-faint);"></i>' +
                                            '<div style="font-weight:700; color:var(--text-primary); margin-top:1rem;">No matching cars</div>' +
